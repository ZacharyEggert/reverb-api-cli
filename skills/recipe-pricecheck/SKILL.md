...
Research active listings and recent sold prices on Reverb.com to establish a used-market price range for gear submitted
for possible purchase. Returns raw data with source links — the buyer makes the final offer judgment.
...
- **Exclude from used-market baseline**: listings where `condition.display_name` is `Brand New` these do not reflect
  second-hand value
- **Flag with caveat, do not exclude**: listings noting replaced parts, repairs, or modifications — note the specific
  caveat alongside the price so the buyer can weigh it — or description indicates unplayed / new-old-stock — these
  should be treated as excellent condition, but the buyer may want to verify the claim
- **Sold > active for baseline**: active listings are asking prices; sold listings are confirmed market transactions —
  weight sold data more heavily
- **Discount depth**: for sold listings with `original_price.display`, note the gap between original and sold price — a
